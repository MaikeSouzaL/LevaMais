...
// List all purposes, optionally filtered by vehicleType/id/isActive
exports.getAll = async (req, res) => {
  try {
    const { vehicleType, id, isActive, limit, skip } = req.query;
...
// Compatibility: GET /api/purposes/:vehicleType (motorcycle|car|van|truck)
exports.getByVehicleType = async (req, res) => {
  try {
    ...
    console.error("Error getting purposes by vehicleType:", error);
    res.status(500).json({ message: "Erro ao buscar tipos de serviço" });
  }
};

// Compatibility: GET /api/purposes/item/:id
// (busca por slug custom "id"; pode filtrar por vehicleType via query)
exports.getById = async (req, res) => {
  try {
    ...
    console.error("Error getting purpose by id:", error);
...
exports.create = async (req, res) => {
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({ message: `O ID "${id}" já existe para ${vehicleType}.` });
    }
...
exports.delete = async (req, res) => {
  try {
    ...
    if (!vehicleType) {
      return res
        .status(400)
        .json({ message: "VehicleType é obrigatório para exclusão" });
    }
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    for (const [vt, ids] of map.entries()) {
      const result = await Purpose.deleteMany({
        vehicleType: vt,
        id: { $in: ids },
      });

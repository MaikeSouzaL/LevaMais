...
export function DeliveryOfferCard({ offer, onAccept, onCounterOffer }: DeliveryOfferCardProps) {
  ...
  const distanceNum = useMemo(() => {
    const parsed = parseFloat(String(distanceKm).replace(/,/g, ".").replace(/[^0-9.]/g, ""));
  ...
  // ==========================================
  return (
    <View className="absolute bottom-0 left-0 right-0 bg-[#0B1523] border-t border-white/10 shadow-3xl rounded-t-[40px] h-[82%]">
      {/* Handle Bar indicator */}
      <View className="w-12 h-1.5 bg-white/10 self-center rounded-full mt-4 mb-1" />
      ...
      >
        ...
        >
          {/* 🤖 AI MATCH BADGE */}
          <View className="self-start bg-[#02de95]/10 border border-[#02de95]/20 px-3 py-1.5 rounded-full flex-row items-center mb-4">
          ...
          {/* 🚨 CRITICAL MISSING INFO COMPONENT (REQUIRED BY USER) 🚨 */}
          ...
          {/* 👥 AUXILIARES & OBS (CONDITIONAL) */}
          <View className="flex-row flex-wrap gap-3 mb-6">
            <View className="flex-1 bg-white/[0.02] border border-white/5 rounded-xl p-3 flex-row items-center">
              <Users size={16} color={helperRequired ? "#02de95" : "rgba(255,255,255,0.2)"} className="mr-2" />
              <View>
                <Text className="text-white/30 text-[8px] font-black uppercase tracking-wider">Ajudante</Text>
                <Text className={`text-xs font-extrabold ${helperRequired ? "text-[#02de95]" : "text-white/60"}`}>
                  {helperRequired ? "Necessário" : "Não Precisa"}
                </Text>
              </View>
            </View>
            {!!obs && (
              <View className="flex-1 bg-white/[0.02] border border-white/5 rounded-xl p-3 flex-row items-center">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-white/60 text-xs font-extrabold" numberOfLines={1}>{obs}</Text>
                </View>
              </View>
            )}
          </View>
          ...
          <View className="flex-row gap-2.5 mb-6">
            <View className="flex-1 bg-white/[0.01] border border-white/5 rounded-xl p-3">
              ...
              <Text className="text-white font-bold text-sm">
                R$ {(counterValue / Math.max(1, distanceNum)).toFixed(2).replace(".", ",")}/km
              </Text>
            </View>
            <View className="flex-1 bg-white/[0.01] border border-white/5 rounded-xl p-3">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.9}
            className={`w-full h-16 rounded-2xl items-center justify-center flex-row relative overflow-hidden shadow-2xl ${
              hasProposedNewValue ? "bg-[#02de95]" : "bg-[#02de95]"
            } shadow-[#02de95]/20`}
          >
            {hasProposedNewValue && (
              <MotiView

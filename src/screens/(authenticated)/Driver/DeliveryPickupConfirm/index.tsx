...
import { CheckCircle, AlertCircle, Package, Phone } from "lucide-react-native";
...
export default function DeliveryPickupConfirm() {
  ...
  const pinCorrect = !hasPin || (pin.length > 0 && pin === ride.details.pickupPin);
  const canConfirm = (!hasPin || pinCorrect) && !uploading;

  const handlePinChange = (value: string) => {
    setPin(value);
    if (hasPin && value.length >= String(ride.details.pickupPin).length) {
      setPinError(value !== ride.details.pickupPin);
    } else {
      setPinError(false);
    }
  };
  ...
  const handleConfirm = async () => {
    ...
    setUploading(true);
    try {
      // Valida PIN via backend
      if (ride?.details?.pickupPin) {
        const pinResult = await rideService.validatePin(rideId, "pickup", pin);
        if (!pinResult?.valid) {
    ...
    <SafeAreaView className="flex-1 bg-[#091A2F]">
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="px-4 py-4 border-b border-[rgba(255,255,255,0.1)]">
          <Text className="text-white text-2xl font-bold">Confirmacao de Coleta</Text>
          <Text className="text-[rgba(255,255,255,0.6)] text-sm mt-1">
            Confirme que voce coletou o pacote
          </Text>
        </View>

...
export const DashboardView = ({
  ...
  const [alertMessage, setAlertMessage] = useState({ title: '', message: '', type: 'origin' as 'origin' | 'dest' });
  const navigation = useNavigation<any>();

  useEffect(() => {
    console.log("[DashboardView] Props alteradas:", {
      hasPickup: !!pickup?.latitude,
      hasDropoff: !!dropoff?.latitude,
      pickupAddr: userAddress,
      dropoffAddr: destinationAddress
    });
  }, [pickup, dropoff, userAddress, destinationAddress]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const pricingRules =
        cityId && results.length > 3 ? (results[3] as PricingRule[]) : [];

      setFavorites(favList);
      setRecentRides(ridesHistory?.rides || []);

      // Check Default Address (Casa ou primeiro da lista)
      const defaultFav = favList.find(
        (f: FavoriteAddress) => f.name?.toLowerCase() === "casa",
      );
      if (defaultFav && onDefaultAddressFound) {
        onDefaultAddressFound(
          defaultFav.address || defaultFav.formattedAddress || defaultFav.name,
        );
      }
      ...
      if (allPurposes && Array.isArray(allPurposes)) {
        const updatedVehicles = INITIAL_VEHICLES.map((v) => {
          ...
          const vehicleServices = allPurposes.filter(
            (p) => p.vehicleType === v.id,
          );

          // Encontrar regra genérica para o veículo (sem purpose específico)
          const genericRule = pricingRules.find(
            (r) =>
              r.vehicleCategory === v.id &&
              (!r.purposeId || r.purposeId === null),
          );
          ...
          return {
            ...
            services: vehicleServices.map((s) => {
              ...
              const startPrice = ruleToUse
                ? ruleToUse.pricing.minimumFee || ruleToUse.pricing.basePrice
                : null;
  ...
  const handleVehiclePress = (vehicle: any) => {
    ...
    const hasDropoff = !!(dropoff?.latitude && dropoff?.longitude && destinationAddress);

    console.log("[DashboardView] handleVehiclePress:", { hasPickup, hasDropoff, destinationAddress });

    if (!hasPickup) {
      setAlertMessage({
        title: "Ponto de Partida",
        message: "Para onde vamos enviar o motorista? Selecione seu endereço de partida primeiro.",
        type: 'origin'
      });
    ...
    if (!hasDropoff) {
      setAlertMessage({
        ...
        message: "Para onde vamos? Selecione o endereço de destino para calcularmos o valor.",
        type: 'dest'
      });
      setAlertVisible(true);
      return;
    ...
    // Cria uma cópia limpa do objeto vehicle para evitar erro de serialização (iconLib é uma função/componente)
      ...
      <View style={styles.header}>
        ...
        <View style={styles.locationInputContainer}>
          ...
          <TouchableOpacity onPress={onPressAddress} style={styles.inputRow}>
            ...
            <Text
              style={[styles.inputText, !userAddress && styles.placeholderText]}
              numberOfLines={1}
            >
      ...
      >
        {loadingServices ? (
          ...
          <View style={{ alignItems: "center", marginTop: 40 }}>
            ...
            />
            <Text
              style={{ color: "#ccc", marginBottom: 16, textAlign: "center" }}
            >
              {errorMsg}
            </Text>
            <TouchableOpacity
            ...
            >
              <Text style={{ color: "#02de95", fontWeight: "bold" }}>
                Tentar Novamente
              </Text>
            </TouchableOpacity>
          </View>
              ...
              return (
                ...
                >
                  <View
                    style={[
                      styles.iconCircle,
                      !hasServices && { backgroundColor: "#444" },
                    ]}
                  >
                    <vehicle.iconLib
                      name={vehicle.icon as any}
                      size={32}
                      color={hasServices ? "#fff" : "#888"}
                    />
                  </View>
                  <Text
                    style={[
                      styles.vehicleLabel,
                      !hasServices && { color: "#666" },
                    ]}
                  >
                    {vehicle.label}
                  </Text>
                  ...
                  {hasServices && (
                    <View style={styles.badgeCount}>
                      <Text style={styles.badgeText}>
                        {vehicle.services.length} opções
                      </Text>
                    </View>
                  )}
                  {!hasServices && (
          ...
          {favorites.length === 0 ? (
            ...
            >
              ...
              <Text style={styles.emptyFavTitle}>
                Adicione seus endereços favoritos
              </Text>
              ...
              <View style={styles.emptyFavButton}>
                ...
                <Text style={styles.emptyFavButtonText}>
                  Adicionar Endereço
                </Text>
              </View>
            </TouchableOpacity>
          ...
          <View style={styles.historySection}>
            ...
            <View style={{ marginTop: 12 }}>
              {recentRides.map((ride) => {
                ...
                const date = created
                  ? format(parseISO(created), "dd 'de' MMM", { locale: ptBR })
                  : "";
                return (
      ...
      <View style={styles.alertOverlay}>
        <View style={styles.alertContent}>
          <View style={[
            ...
            { backgroundColor: alertMessage.type === 'origin' ? 'rgba(2,222,149,0.1)' : 'rgba(239, 68, 68, 0.1)' }
          ]}>
            <MaterialIcons 
              name={alertMessage.type === 'origin' ? "location-on" : "flag"} 
              size={32} 
              color={alertMessage.type === 'origin' ? "#02de95" : "#ef4444"} 
            />
          </View>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              ...
              if (alertMessage.type === 'origin') onPressAddress();
              else onPressDestination();
            }}
          >
          ...
          >
            <Text style={styles.alertCancelText}>Depois</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
